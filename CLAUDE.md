...
src/
  ...
  hooks/
    ...
    useVkTheme.ts       # VK color-scheme detection + body mirror; inits bridge, banner ad
...
- All VK Bridge calls are wrapped with error handling and graceful no-ops for non-VK environments (note: `bridge.send` returns a Promise — use `.catch`, not a synchronous try/catch).
